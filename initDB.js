...
function dbHandler(err, res){
    if(err) console.log(err);
    console.log(res);
}


db.getConnection().query("USE `chess`;",dbHandler);

db.getConnection().query("DROP TABLE IF EXISTS `users`;",dbHandler);
...
db.getConnection().query("CREATE TABLE `users` ( \
  ...
  ) ENGINE=InnoDB AUTO_INCREMENT=1 COMMENT='Utilisateurs de l''application';",dbHandler);

  db.getConnection().query("DROP TABLE IF EXISTS `chessgame`;",dbHandler);
  db.getConnection().query("CREATE TABLE `chessgame` ( \
